...
public class ImageSFXs
{
     ...
     */
    {
        ...
        ConvolveOp blurringOp = new ConvolveOp(new Kernel(size, size, blurKernel), ConvolveOp.EDGE_NO_OP, null); // leaves edges unaffected
     ...
     * 
     * if (brightness < 0.0f) { System.out.println("Brightness must be >= 0.0f; setting to 0.0f"); brightness = 0.0f; } // brightness may be less than
     * 1.0 to make the image less red
     * 
     * RescaleOp brigherOp; if (hasAlpha(im)) { float[] scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness, 1.0f}; // don't change alpha //
     * without the 1.0f the RescaleOp fails, which is a bug (?) float[] offsets = {64.0f, 0.0f, 0.0f, 0.0f}; brigherOp = new RescaleOp(scaleFactors,
     * offsets, null); } else { // not transparent float[] scaleFactors = {brightness, 1.0f/brightness, 1.0f/brightness}; float[] offsets = {64.0f,
     * 0.0f, 0.0f}; brigherOp = new RescaleOp(scaleFactors, offsets, null); } g2d.drawImage(im, brigherOp, x, y); } // end of drawRedderImage() using
     * RescaleOp
     */
    ...
    /* Mix up the colours in the green and blue bands of the image. */
    {
        ...
        if (hasAlpha(im))
        {
            float[][] colourMatrix = { { 1.0f, 0.0f, 0.0f, 0.0f }, // for new red band, unchanged
        ...
        { // not transparent
            float[][] colourMatrix = { { 1.0f, 0.0f, 0.0f }, // for new red band, unchanged
     ...
     */
    {
        ...
        BufferedImage dest = new BufferedImage(src.getWidth(), src.getHeight(), BufferedImage.TYPE_INT_ARGB); // alpha channel
        Graphics2D g2d = dest.createGraphics();
